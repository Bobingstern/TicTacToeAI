...
class DuelingDeepQNetwork(keras.Model):
    def __init__(self, n_actions, fc1_dims, fc2_dims):
        ...
        self.dense3 = keras.layers.Dense(fc2_dims, activation='relu')
        self.dense4 = keras.layers.Dense(fc2_dims, activation='relu')
        self.V = keras.layers.Dense(1, activation=None)
        self.A = keras.layers.Dense(n_actions, activation=None)

    def call(self, state, isAdvantage=False):
        x = self.dense1(state)
        x = self.dense2(x)
        x = self.dense3(x)
        x = self.dense4(x)
...
class Agent():
    ...
    def choose_action(self, observation):
        if np.random.random() < self.epsilon:
            ...
            actions = self.q_eval.call(state, True)
    ...
    def save_models(self):
        if not self.can_save:
            return False
        self.q_next.save("models/nextG")
        self.q_eval.save("models/evalG")
        self.q_eval.save_weights("models/ticer2")
...
class AgentDQN():
    def __init__(self, lr, gamma, n_actions, epsilon, batch_size,
        ...
        self.memory = ReplayBufferDQN(mem_size, input_dims)
        self.q_eval = build_dqn(lr, n_actions, input_dims, 64, 64)
        self.q_next = build_dqn(lr, n_actions, input_dims, 64, 64)
        self.canSave = False
    def store_transition(self, state, action, reward, new_state, done):
        self.memory.store_transition(state, action, reward, new_state, done)
    ...
    def save_model(self):
        if not self.canSave:
            return False
        self.q_eval.save("models/dqnSave")
        tfjs.converters.save_keras_model(self.q_eval, 'tfjs_models')


    def load_model(self):
        self.q_eval = load_model("models/dqnSaveEp2")
